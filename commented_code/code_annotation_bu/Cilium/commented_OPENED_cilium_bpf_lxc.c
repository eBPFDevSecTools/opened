...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: check if the proxy is able to be redirected, and redirect the proxy, store in status. Return true if successful.
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: encode meta data (return value and identity), and store it into ctx buffer
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: Handling egress ipv6 traffic by 
 a) lookup the destination_id. For encrypted and tunneled traffic b) tunnel endpoint, and c) encryption key as well.
 Also mark the traffic if it has a remote endpoint destination.
 Restore ct_state from per packet lb handling in the previous tail call. Enable the per pecket load balancer, swap the address by CT_lookup and establish the connection.
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: Count ipv6 tail calls
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: Handle tail messages. Check if it is not special ICMPv6 messages such as echo requests,
 neighbour advertisement, then check if the destination address is among the address that should be 
 load balanced. Then store information to ctx for continuous tail call.
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: Handle egress IPv6 traffic from a container after service translation has been done
 either at the socket level or by the caller.
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: Handling egress ipv4 traffic. Mark the traffic if it has a remote endpoint destination.
 Restore ct_state from per packet lb handling in the previous tail call. Enable the per pecket load balancer, swap the address by CT_lookup and establish the connection.
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: Count ipv4 tail call. Helper function for handling ipv4 traffic
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: helper function to handle ipv4 tail call
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: handle ipv4 tail call.
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: Validate ARP requests. Send unknown and LXC endpoint ARP requests to linux kernel stack. 
 Send response for all other ARP requests.
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: Use previous exgress ipv6/v4 traffic handler to handle exgress traffic.
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: Check if the packet is ingress or exgress traffic, redirect the traffic if necessary.
 OPENED COMMENT END 
 */ 
static __always_inline int
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: Identify ipv6 tail call, store data to ctx. Skip tail call when packet is being redirected to a L7 proxy.
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: delivery ipv6 tail call to endpoint, skip if it is send to L7 proxy
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: Identify ipv4 message and store its meta data to ctx.
 OPENED COMMENT END 
 */ 
static __always_inline int
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: Redirect ipv4 tail call, skip tail calls when packet is being redirected to L7 proxy.
 OPENED COMMENT END 
 */ 
declare_tailcall_if(__and(is_defined(ENABLE_IPV4), is_defined(ENABLE_IPV6)),
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: Send ipv4 tail call to its enpoint.
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: Identify if the traffic is ipv6/v4, handle policy using previous helper function.
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: Handle egress traffic 
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: handle ingress policy by checking it's source and destination stored in ctx buffer.
 Check identity and drop the unsupported and tail calls.
 OPENED COMMENT END 
 */ 
