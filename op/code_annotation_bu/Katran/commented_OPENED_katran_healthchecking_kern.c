...
/*
 ...
 Update_maps: []
 Func Description: Input is user accessible mirror of in-kernel sk_buff
                   This function performs healthcheck for encapsulation
                   ...
                   The program returns error/healthcheck fails (TC_ACT_SHOT) if 1)packet is bigger than the specified size,
                   2)do not have ifindex for main interface,
                   3)do not find HC_SRC_MAC_POS or HC_DST_MAC_POS,
                   4)the result for HC_ENCAP (healthcheck encap) is faulse.
                   Otherwise, the packet passes the healthcheck, and will be redirected to another net device of index intf_ifindex.
