...
/* 
 ...
 Output: int 
 Helpers: [bpf_clear_meta,validate_ethertype] 
 Read_maps: [] 
 Update_maps: [ctx] 
 Func Description: Initialize the ctx buffer, passing protocols to the stack according
 to the imput packets (ESP packets coming from network; Non-ESP packets coming from network;
 Non-ESP packets coming from stack re-inserted by xfrm)
 OPENED COMMENT END 
 */ 
