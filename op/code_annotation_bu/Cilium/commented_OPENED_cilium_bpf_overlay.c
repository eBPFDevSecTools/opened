...
/* 
 ...
 Output: int 
 Helpers: [revalidate_data_pull bpf_skip_nodeport nodeport_lb6 encap_remap_v6_host_address get_identity ctx_get_tunnel_key identity_is_remote_node ipcache_lookup6 
 cilium_dbg update_metrics set_identity_mark ctx_change_type send_trace_notify lookup_ip6_endpoint ipv6_hdrlen ipv6_local_delivery ipv6_l3 cilium_dbg_capture ctx_redirect] 
 Read_maps: [] 
 Update_maps: [] 
 Func Description: The main goal of handle_ipv6 is to check the different condition including 
 if nodeport, IPSEC is enabled and if the pulling data or decrypting gets error. 
 Then make different action to handle according situatoins to convert the IPV6 to a virtual 
 network which points to overlay. The first step is to verify the background by 
 using revalidate_data_pull and the pointer ctx (maybe the starting point of a protocol?). 
 Ret should be the new address of this IPV6 sector. If nodeport is not activated, 
 ret is set by encap_remap_v6_host address.
Then check if the net is decrypted by using mark. Identity is equal to identity of ctx if 
the net is decrypted, but tunnel id if not. Check if the ID is equal to host_ID since any 
node encapsulating will map any HOST_ID source to be presented as REMOTE_NODE_ID, 
therefore any attempt to signal HOST_ID as source from a remote node can be dropped.
After that, check esp protocol,ESPis a member of the Internet Protocol Security set 
of protocols that encrypt and authenticate the packets of data between computers using 
a Virtual Private Network. If ESP is activated, we set mark for the ID and change the 
type of ctx to packet_host to pass it up.
...
static __always_inline int handle_ipv6(struct __ctx_buff *ctx,
				       __u32 *identity)
{
	int ret, l3_off = ETH_HLEN, hdrlen;         //rth_hlen means total octets in header
	void *data_end, *data;
	struct ipv6hdr *ip6;
	...
	if (!decrypted) {
		 ...
		 */
		if (ip6->nexthdr != IPPROTO_ESP) {                               //ESP) is a member of the Internet Protocol Security (IPsec) set of protocols that encrypt and authenticate the packets of data between computers using a Virtual Private Network (VPN).
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: This function is to handle the tail of a protocol and check it it has any error for ipv6
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Input: (struct __ctx_buff *ctx, __u32 *identity) 
 Output: int 
 Helpers: [revalidate_data_pull ipv4_is_fragment nodeport_lb4  get_identity ctx_get_tunnel_key identity_is_remote_node ipcache_lookup4 
 cilium_dbg update_metrics set_identity_mark ctx_change_type send_trace_notify lookup_ip4_endpoint ipv4_local_delivery ipv4_l3 cilium_dbg_capture ctx_redirect] 
 Read_maps: [] 
 Update_maps: [] 
 Func Description: This function is similar to handle_ipv6 to handle ipv4 packet. The main goal of it is to check the different condition including 
 if nodeport, IPSEC is enabled and if the pulling data or decrypting gets error. 
 Then make different action to handle according situatoins to convert the IPV6 to a virtual 
 network which points to overlay. IPV4 fragmentation check is added.
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Input: (struct __ctx_buff *ctx) 
 Output: int 
 Helpers: [handle_ipv4 IS_ERR send_drop_notify_error] 
 Read_maps: [] 
 Update_maps: [] 
 Func Description: check if there is error about the tail of ipv4 packet.
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Read_maps: [] 
 Update_maps: [] 
 Func Description: Handle the tail for ARP requests from VTEP. Check different occasions including tunnel_key geting, arp validating, and if endpoint of tunnel is reached.
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Output: bool 
 Helpers: [bpf_htons revalidate_data_pull] 
 Read_maps: [] 
 Update_maps: [] 
 Func Description: The built in function to check if esp protocol is used and if pulling data have error.
 OPENED COMMENT END 
 */ 
...
/* 
 ...
 Input: (struct __ctx_buff *ctx) 
 Output: int 
 Helpers: [bpf_clear_meta bpf_skip_nodeport_clear validate_ethertype send_trace_notify bpf_htons ep_tail_call] 
 Read_maps: [] 
 Update_maps: [] 
 Func Description: handle different possible packets come to the program. First check if it is esp protocol,
 then check the decrypted or not and get the identity from the metadata. 
 OPENED COMMENT END 
 */ 
